...
class TkComboBox(QtWidgets.QComboBox, Menu, Attributes):
	...
	def showPopup(self):
		...
		super(TkComboBox, self).showPopup()
	...
	def hidePopup(self):
		...
		if not self.popupStyle=='modelView':
			...
			super(TkComboBox, self).hidePopup()
	...
	def eventFilter(self, widget, event):
		...
		if event.type()==QtCore.QEvent.Hide:
			if self.parent().__class__.__name__=='TkMenu':
				self.parent().hide()

		return super(TkComboBox, self).eventFilter(widget, event)
...
# def children_(self, of_type=[], contextMenu=False, _exclude=['QAction', 'QWidgetAction']):
	...
	# 		self._menu = TkMenu(self, position='cursorPos')
	# 	return self._menu
