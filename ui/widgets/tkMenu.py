...
class TkMenu(QtWidgets.QMenu, Attributes):
	'''
	:Parameters:
...
if __name__ == "__main__":
	...
	m = TkMenu(position='cursorPos', setVisible=True)
	m1 = TkMenu('Create', addMenu_=m)
	m1.add('QAction', setText='Action', insertSeparator_=True)
	m1.add('QAction', setText='Action', insertSeparator_=True)
	m1.add('QPushButton', setText='Button')

	m2 = TkMenu('Cameras', addMenu_=m)
...
# 		return self._contextMenuToolTip
	...
	# 	ex. usage: 
	# 	m = TkMenu(position='cursorPos', setVisible=True)
	# 	m1 = TkMenu('Create', addMenu=m)
	# 	m2 = TkMenu('Cameras', addMenu=m)
	# 	m.addMenus([m1,m2])
	# 	'''
