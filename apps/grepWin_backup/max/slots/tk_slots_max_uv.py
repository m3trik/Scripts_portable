...
class Uv(Init):
	...
	def chk016(self):
		...
		mod = self.uv_ui_uiModifier #get/set the uv modifier.
	...
	def tb000(self, state=None):
		...
		self.uv_ui_uiModifier.pack(1, 0.01, scale, rotate, True) #(method, spacing, normalize, rotate, fillholes)
	...
	def tb001(self, state=None):
		...
		for obj in objects:
			if standardUnwrap:
				try:
					self.uv_ui_uiModifier.FlattenBySmoothingGroup(scaleMode, False, 0.2)
	...
	def tb004(self, state=None):
		...
		# if optimize:
		# 	# self.uv_ui_uiModifier.
		# else:
		self.uv_ui_uiModifier.relax(1, 0.01, True, True)
	...
	def tb005(self, state=None):
		...
		self.uv_ui_uiModifier.Straighten()
	...
	def b005(self):
		'''Cut Uv'S
		'''
		self.uv_ui_uiModifier.breakSelected()


	def b011(self):
		'''Sew Uv'S
		'''
		self.uv_ui_uiModifier.stitchVerts(True, 1.0) #(align, bias) --Bias of 0.0 the vertices will move to the source and 1.0 they will move to the target. 
	...
	def flipUV(objects=None):
		...
		for obj in objects:
			try:
				uv = self.uv_ui_uiModifier #get/set the uv xform modifier.
				uv.U_Flip = u
				uv.V_Flip = v
				uv.W_Flip = w
