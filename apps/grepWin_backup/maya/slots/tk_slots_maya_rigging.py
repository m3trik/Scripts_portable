...
class Rigging(Init):
	...
	def draggable_header(self, state=None):
		'''Context menu
		'''
		draggable_header = self.rigging.draggable_header

		if state is 'setMenu':
			draggable_header.contextMenu.add(wgts.TkComboBox, setObjectName='cmb000', setToolTip='Maya Rigging Editors')
			return
	...
	def chk000(self, state=None):
		...
		self.rigging.tb000.menu_.s000.setValue(pm.jointDisplayScale(query=1)) #init global joint display size
	...
	def chk001(self, state=None):
		...
		self.rigging.s000.setValue(pm.ikHandleDisplayScale(query=1)) #init IK handle display size
	...
	def chk002(self, state=None):
		...
		self.rigging.s000.setValue(pm.jointDisplayScale(query=1, ikfk=1)) #init IKFK display size


	def s000(self, value=None):
		'''Scale Joint/IK/FK
		'''
		value = self.rigging.s000.value()

		if self.rigging.chk000.isChecked():
			pm.jointDisplayScale(value) #set global joint display size
		elif self.rigging.chk001.isChecked():
			pm.ikHandleDisplayScale(value) #set global IK handle display size
		else: #self.rigging.chk002.isChecked():
