...
class Uv(Init):
	...
	def draggable_header(self, state=None):
		'''Context menu
		'''
		draggable_header = self.uv_ui_ui.draggable_header
	...
	def cmb001(self, index=None):
		...
		if index is 'setMenu':
			...
			try:
				...
				cmb.menu_.add(wgts.TkCheckBox, setObjectName='chk014', setText='Checkered', setChecked=checkered, setToolTip='')
				cmb.menu_.add(wgts.TkCheckBox, setObjectName='chk015', setText='Borders', setChecked=borders, setToolTip='')
				cmb.menu_.add(wgts.TkCheckBox, setObjectName='chk016', setText='Distortion', setChecked=distortion, setToolTip='')
	...
	def tb000(self, state=None):
		...
		if state is 'setMenu':
			tb.menu_.add(wgts.TkCheckBox, setText='Pre-Scale Mode: 1', setObjectName='chk025', setTristate=True, setCheckState_=1, setToolTip='Allow shell scaling during packing.')
			tb.menu_.add(wgts.TkCheckBox, setText='Pre-Rotate Mode: 1', setObjectName='chk007', setTristate=True, setCheckState_=1, setToolTip='Allow shell rotation during packing.')
			...
			tb.menu_.add(wgts.TkCheckBox, setText='Stack Similar: 2', setObjectName='chk026', setTristate=True, setCheckState_=2, setToolTip='Find Similar shells. <br>state 1: Find similar shells, and pack one of each, ommiting the rest.<br>state 2: Find similar shells, and stack during packing.')
	...
	def tb001(self, state=None):
		...
		standardUnwrap = self.uv_ui_ui.chk000.isChecked()
		scaleMode = self.uv_ui_ui.chk001.isChecked()
		seamOnly = self.uv_ui_ui.chk002.isChecked()
		planarUnwrap = self.uv_ui_ui.chk003.isChecked()
		cylindricalUnwrap = self.uv_ui_ui.chk004.isChecked()
		sphericalUnwrap = self.uv_ui_ui.chk005.isChecked()
		normalBasedUnwrap = self.uv_ui_ui.chk006.isChecked()
	...
	def tb003(self, state=None):
		...
		back_facing = self.uv_ui_ui.chk008.isChecked()
		front_facing = self.uv_ui_ui.chk009.isChecked()
		overlapping = self.uv_ui_ui.chk010.isChecked()
		nonOverlapping = self.uv_ui_ui.chk011.isChecked()
		textureBorders = self.uv_ui_ui.chk012.isChecked()
		unmapped = self.uv_ui_ui.chk013.isChecked()
